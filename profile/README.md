<div align="center" style="border: none;">
<!-- Main Logo -->
<img src="https://raw.githubusercontent.com/Croissant-Engineering/.github/main/profile/ce_logo_github.png" alt="Croissant Engineering Logo" width="220" style="border-radius: 50%; border: none;" />
<br />

# Croissant Engineering
**Croissant Engineering** designs complex software systems from seemingly simple components, much like the intricate layering of a **perfect croissant**. We engineer robust, scalable, and beautifully crafted solutions.
</div>

---

<br />

### Projects:
<table width="100%" style="border-collapse: collapse; border: none;">
  <tr style="border: none; background: none;">
    <!-- Project 1: FlowLinX -->
    <td width="33%" align="center" valign="top" style="border: none; padding: 20px;">
      <br /><br />
      <img src="https://raw.githubusercontent.com/Croissant-Engineering/.github/main/profile/fwlnx_logo_github.png" alt="FlowLinX Logo" width="151" height="151" style="border-radius: 50%; margin-bottom: 10px; object-fit: cover;" />
      <br />
      <div style="margin-top: 10px; margin-bottom: 10px;">
        <img src="https://raw.githubusercontent.com/Croissant-Engineering/.github/refs/heads/main/profile/flowlinx_wordmark.svg" alt="FlowLinX" width="220" />
      </div>
      <p style="font-size: 14px; color: #8b949e; line-height: 1.5;">Bring your Flow project — playlists, shots/assets, versions, notes and tasks —
into xStudio, and publish playlists and new notes with annotations back to Flow.</p>
      <br /><br />
    </td>
    <!-- Project 2: Auto-Segmentation -->
    <td width="33%" align="center" valign="top" style="border: none; padding: 20px;">
      <br /><br />
      <img src="https://raw.githubusercontent.com/Croissant-Engineering/.github/main/profile/autoseg_logo_circles.svg" alt="xStudio Auto-Segmentation Logo" width="151" height="151" style="border-radius: 50%; margin-bottom: 10px; object-fit: cover;" />
      <br />
      <div style="margin-top: 10px; margin-bottom: 10px;">
        <img src="https://raw.githubusercontent.com/Croissant-Engineering/.github/main/profile/autoseg_logo_text.svg" alt="xStudio Auto-Segmentation" width="220" />
      </div>
      <p style="font-size: 14px; color: #8b949e; line-height: 1.5;"></p>
      <br /><br />
    </td>
    <!-- Project 3: xSplatting -->
    <td width="33%" align="center" valign="top" style="border: none; padding: 20px;">
      <br /><br />
      <svg width="290" height="34" viewBox="0 0 290 34" style="display: block; margin: 0 auto;">
        <text x="0" y="26" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" font-size="28" font-weight="900" letter-spacing="-0.5">
          <tspan fill="#17768D">xStudio Gaussian</tspan><tspan fill="#F3795C">Splatting</tspan>
        </text>
      </svg>
      <p style="font-size: 14px; color: #8b949e; line-height: 1.5;"></p>
      <br /><br />
    </td>
  </tr>
</table>

<br />

<div align="center">
  <sub>© 2026 Croissant Engineering. All rights reserved.</sub>
</div>
